 ...
 *             } 
 */
// router.get('/savedGraphs/:id', async (req, res) => {
//     logger.info(`START GET /graphData/savedGraphs/${req.params.id}`, req.body);
//     const id = req.params.id;
//     let dbconfig: DbConfig[] = [{dbpath: config().stardogSavedGraph.path,
//                                 selectedNamedGraphs: [ '' ],
//                                 instance: 'stardogSavedGraph',
//                                 virtualGraphs: [] }
//                                 ]
    
//     const graphData = new GraphDataService(config().stardogSavedGraph, dbconfig);
//     const result = await graphData.getSavedGraphData(id);
//     if (result) {
...
//     } 
//     else {
//         let dbconfig: DbConfig[] = [{dbpath: config().stardogSavedGraph.updatePath,
//             selectedNamedGraphs: [ '' ],
//             instance: 'stardogSavedGraph',
//             virtualGraphs: [] }
//             ]
//         const graphData = new GraphDataService(config().stardogSavedGraph, dbconfig);
//         const result = await graphData.postSaveGraphData(body.id, body.data);
//         if (result) {
...
router.get('/virtualGraphs', async (req, res) => {
    ...
    return null;
    // const triplestoreNames = Object.keys(config().tripleStores);
    // const triplestore = config().tripleStores[triplestoreNames[0]];
    ...
    //     res.status(200).json(result);
    // } else {
    //     result = await graphData.getVirtualGraphs({ [triplestoreNames[0]]: triplestore});
    //     if (result) {
    //         logger.info(`FINISH GET /graphData/virtualGraphs`, {status: 200});
    //         res.status(200).json(result);
    //     }
...
router.get('/dbpaths', async (req, res) => {
    ...
    const result = await graphData.getTripleStores();
